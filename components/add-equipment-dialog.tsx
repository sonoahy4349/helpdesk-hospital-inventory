  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.tipo === "impresora") {
      if (!formData.perfil || !formData.tipoImpresora) {
        toast({
          title: "Error de validación",
          description: "Para impresoras, los campos Perfil y Tipo son obligatorios.",
          variant: "destructive",
        })
        setIsSubmitting(false)
        return
    ...
    try {
      ...
      toast({
        title: "Error",
        description: `No se pudo agregar el equipo: ${error.message}`,
        variant: "destructive",
      })
    } finally {
      setIsSubmitting(false)
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>Completa la información del nuevo equipo para agregarlo al inventario.</DialogDescription>
        </DialogHeader>
