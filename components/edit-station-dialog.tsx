...
export function EditStationDialog({ open, onOpenChange, station, onStationUpdated }: EditStationDialogProps) {
  ...
  useEffect(() => {
    if (station) {
      ...
      setFormData({
        ...
        ubicacion: station.location_id?.toString() || "",
        responsable: station.responsible_id?.toString() || "",
        autorizacion: station.authorized_by || "",
      ...
      setFormData((prev: any) => ({
        ...prev,
        equipoPrincipal: primaryEquipment?.equipment_id?.toString() || "",
        equipoSecundario: secondaryEquipment?.equipment_id?.toString() || "",
        equipoTercero: tertiaryEquipment?.equipment_id?.toString() || "",
      }))

      const currentAccessories: any = {
        mouse: false,
        teclado: false,
        webcam: false,
        audifonos: false,
      ...
      station.workstation_accessories?.forEach((acc: any) => {
        if (acc.included) {
          currentAccessories[acc.accessory_type] = true
        }
      })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || "Error al actualizar estación")
      }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle>Editar Estación {station?.id}</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="grid gap-4 py-4">
          <div className="space-y-2">
            <Label htmlFor="stationType">Tipo de Estación</Label>
            <Select onValueChange={setStationType} value={stationType} disabled>
          ...
          <div className="space-y-2">
            ...
            <Select onValueChange={(value) => handleSelectChange("status", value)} value={formData.status || "active"}>
