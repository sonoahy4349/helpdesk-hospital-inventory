...
interface EditResponsibleDialogProps {
  ...
  responsible: any
  onResponsibleUpdated: () => void // Callback to notify parent
}

export function EditResponsibleDialog({
  open,
  onOpenChange,
  responsible,
  ...
  useEffect(() => {
    if (responsible && open) {
      setFormData({
        nombreCompleto: responsible.nombre_completo || "",
        cargo: responsible.cargo || "sin-asignar",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/responsibles/${responsible.id}`, {
        method: "PUT",
        headers: {
      ...
      const updatedResponsible = await response.json()
      console.log("Responsable actualizado:", updatedResponsible)
      onResponsibleUpdated() // Notify parent to re-fetch data
      onOpenChange(false) // Close dialog
    } catch (err: any) {
      console.error("Failed to update responsible:", err)
      toast({
        title: "Error al actualizar",
        description: "No se pudo actualizar el responsable: " + err.message,
        variant: "destructive",
      })
    } finally {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>Editar Responsable #{responsible.id}</DialogTitle>
          <DialogDescription>Modifica la información del responsable en el sistema.</DialogDescription>
        </DialogHeader>
